...
export function CourtSelectionView({ hlCase, onUpdate, onNext, onBack }: Props) {
  ...
  function selectState(s: string) {
    ...
    if (level) {
      ...
      if (available.length === 1) {
        const court = available[0];
        setSelectedCourt(court);
        // Auto-save when only one court — no extra click needed
        onUpdate({ ...hlCase, court, jurisdiction: court.state, workflowStage: "story" });
      }
    }
  }
  ...
  function handleSelectCourt(court: Court) {
    setSelectedCourt(court);
    // Auto-save immediately when court is chosen
    onUpdate({ ...hlCase, court, jurisdiction: court.state, workflowStage: "story" });
  }
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: BG, color: PAPER, minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        <div style={{ maxWidth: 540, margin: "0 auto", padding: "28px 20px 16px" }}>
          ...
          {/* Why this matters */}
          <WhyThisMatters>
            Every legal document must be filed with a specific court. Federal courts handle civil rights claims under federal law (like § 1983), while state courts handle state-law claims. Selecting the right court now ensures your documents name the correct court — a common pro se mistake is filing in the wrong jurisdiction.
          </WhyThisMatters>
          ...
          {courts.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {courts.map((court, i) => (
                  <button key={i} onClick={() => handleSelectCourt(court)} style={{
          ...
          {level && selectedState && courts.length === 0 && (
            <div style={{ color: "#555", fontSize: 13, padding: "16px", textAlign: "center", marginBottom: 16 }}>
